...
class MainPanel(tk.Frame):
    ...
    def __init__(self):
        ...
        self.b_connect=tk.Button(self.gui, text="x", font=fonts['status'], fg=Colours["darkred"], bg=Colours['grey'], width = 1, height = 1, command = lambda: self.disconnect())
        ...
        self.hour_s = round(getvalue(getaddress("gui","clock_zero"),"u","1")["value"]/6,1)
        if self.hour_s > 150000:
            self.hour_s = 0
        self.hour = round(getvalue("0x0005", "u", "1")["value"]/6,1)
        self.clock_disp = str(round(self.hour-self.hour_s))
        try:
            self.serialnumber=str(getvalue(getaddress("lh", "serial"),lh['serial'][1],lh['serial'][2])['value'])
        except:
            self.serialnumber = "TEST_ENV"
        self.l_serial=tk.Label(self.gui, text="S/N: " + self.serialnumber + ", " + self.clock_disp, font=fonts['detection'], fg="grey", bg=Background['main'])
        ...
        self.l_copy=tk.Label(self.gui, text="All rights reserved", bg=Background['main'], font=fonts['copyright'])
        ...
        if "TEC0" in available:
            ...
            self.l_tec0_temp = tk.Label(self.gui, textvariable=self.tec0_n, font=fonts['temperature'], bg=Background['main'])
        ...
        self.b_lon=tk.Button(self.gui, text="SYSTEM ON", bg=Colours['green'], fg=Colours["white"], command=lambda: self.laser_on(), font=fonts['subbutton'], height=1, width=5)
        ...
        self.b_ldon=tk.Button(self.gui, text="PUMP ON", bg=Colours['grey'], fg=Colours["darkgrey"], font=fonts['subbutton'], height=1, width=5)
        #self.b_ldoff = tk.Button(self.gui, text="LD\noff", bg=Colours['grey'], fg=Colours["darkgrey"],font=fonts['subbutton'], height=1, width=5)
        self.b_loff=tk.Button(self.gui, text="SYSTEM OFF",bg=Colours['red'], fg="white", command=lambda: self.laser_off(), font=fonts['subbutton'],height=1, width=10)
        self.b_lock=tk.Button(self.gui, text="RELOCK", fg=Colours["darkgrey"], bg=Colours['grey'], command="", font=fonts['subbutton'], height=1, width=10)
        self.b_alignment=tk.Button(self.gui, text="EXT ALIGNMENT", fg="black", bg=Colours['lightgrey'], command=lambda: self.alignment(), font=fonts['main'], height=1, width=7)
        self.b_highp=tk.Button(self.gui, text="FULL POWER", fg="black", bg=Colours['lightgrey'], command=lambda: self.highp(), font=fonts['main'], height=1, width=7)
        ...
        self.l_detect.grid(row=4, column=1, columnspan=4, rowspan=2, sticky="nws")
        ...
        self.l_messages.grid(row=29, column=1, columnspan=4, rowspan=2, sticky="nw")
    ...
    def on_closing(self):
        if messagebox.askokcancel("Exit", "Are you sure you want to close the GUI?"):
            ...
            if Globals.tec_stab == 1:
                ...
                if readbit(self.actual, status["TEC_READY"]) == "1" and Globals.laser_turnhigh == 1:
                    ...
                    if Globals.opmsetting == 1:
                        ...
                        TARGET_CURR = Globals.Names['high']
                    setvalue(getaddress("ld_d", "curr"), TARGET_CURR, "u", "u")
                    if TARGET_CURR < 0.95*Globals.Names['high'] and TARGET_CURR > Globals.Names['low']:
                        self.park_on()
                    else:
                        self.lock_on()
        ...
        if Globals.laser_off == 0:
            ...
            if getbit(status["address"], status["LD_STABLE"]) == "1":
                ...
                if getbit(status["address"], status["LD_STABLE"]) == "1":
                    ...
                    TARGET_CURR = getvalue(getaddress("ld_d", "curr"), "u", "u")["value"]
                    if TARGET_CURR < 0.95 * Globals.Names['high'] and TARGET_CURR > Globals.Names['low']:
                        self.park_on()
                    else:
                        self.lock_on()
    ...
    def park_on(self):
        ...
        if getbit(status["address"], status["LD_STABLE"]) == "1":
            self.message_trigger("PZT is now parked.\n")
            if "PZT0" in Globals.available:
                control_bit = getvalue(control["address"])["value"]
                if readbit(control_bit, control["pzt0_lock"]) == "1":
                    resvalue(control["address"], 2 ** control["pzt0_lock"])
    ...
    def getnametec(self, base, label, status):
        ...
        Globals.runnning_PROC.append(self.getnametec_after)
