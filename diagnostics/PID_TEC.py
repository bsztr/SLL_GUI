...
T_address = getaddress(TEC+"_d", "set")
...
print ("total days: " + str((TEMP_step*P_step*D_step*D_step*dset)/(10*60*60*24)))
print("total files " + str((TEMP_step*P_step*D_step*D_step*dset)/(1)))
...
for i in TEMP:
    ...
    time.sleep(300)
    for j in P:
        setvalue(P_address, j, "f", "1")
        for k in I:
            setvalue(I_address, k, "f", "1")
            for l in D:
                setvalue(D_address, l, "f", "1")
                output = []
                output2 = []
                for m in range(dset):
