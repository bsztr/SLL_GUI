...
class ClientPanel_NAVY(tk.Frame):
    ...
    def __init__(self):
        ...
        if "PZT0" in Globals.available:
            Globals.regoffset = getvalue(getaddress("pzt0_d", "dpotb_amp"), "u", "1")["value"]
            basing = "pzt0"
        if "PZT1" in Globals.available:
            Globals.regoffset = getvalue(getaddress("pzt1_d", "dpotb_amp"), "u", "1")["value"]
            basing = "pzt1"
        global message
        message = ""
        ...
        self.l_clp_ci = tk.Label(self.gui, text="CLP V-I #", font=fonts['main'], bg=Background['main'])
        self.t_clp_ci = tk.Text(self.gui, width=6, height=1)
        self.s_clp_ci = pzt_label(self.gui)
        self.s_clp_ci.update_status(getaddress(basing + "_d", "clp_ci"), "clp_ci", "driver")
        self.b_clp_ci = tk.Button(self.gui, width=3, height=1, text="OK", font=fonts['submit'],
                                  bg=Background['submit'],
                                  command=lambda: submit(self, basing + "_d", "clp_ci"))
        ...
        self.l_clp_ci.grid(row=21, column=1, columnspan=1, rowspan=1, sticky="nws", padx=(3, 3), pady=(2, 2))
        self.t_clp_ci.grid(row=21, column=2, columnspan=1, rowspan=1, sticky="nws", padx=(3, 3), pady=(2, 2))
        self.s_clp_ci.grid(row=21, column=3, columnspan=1, rowspan=1, sticky="nws", padx=(3, 3), pady=(2, 2))
        self.b_clp_ci.grid(row=21, column=4, columnspan=1, rowspan=1, sticky="nws", padx=(3, 3), pady=(2, 2))
    ...
    def pzt_regscale(self):
        ...
        towrite = Globals.regoffset + result
...
#         tk.Toplevel.__init__(self)#         #self.grab_set()
